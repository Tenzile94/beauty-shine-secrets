...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  const product = products.find(p => p.id === parseInt(id) && p.category === 'skincare');
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="relative">
              <div className="sticky top-24">
                <div className="relative h-[500px] bg-gradient-to-br from-pink-100 to-purple-100 rounded-3xl overflow-hidden shadow-xl flex items-center justify-center p-12">
                  {product.badge && (
                    <div className="absolute top-6 left-6 bg-pink-500 text-white px-4 py-2 rounded-full text-sm font-semibold z-10">
                      {product.badge}
                    </div>
                  )}
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {product.benefits.map((benefit, index) => (
                      <span
                        key={index}
                        className="bg-white border border-pink-200 text-gray-700 px-4 py-2 rounded-full text-sm"
                      >
                        {benefit}
                      </span>
                    ))}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              {product.keyFeatures.map((feature, index) => (
                <div
                  key={index}
                  className="flex items-start gap-3 bg-gradient-to-r from-pink-50 to-purple-50 p-4 rounded-xl"
                >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="space-y-4">
              {product.howToUse.map((step, index) => (
                ...
                >
                  <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-full flex items-center justify-center font-bold">
                    {index + 1}
                  </div>
